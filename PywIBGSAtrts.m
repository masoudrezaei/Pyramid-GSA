function [out]=PywIBGSAtrts(Data,Targets,params,KFindices,K,Kf,pTr,classMethod,Fnum1w,DataT, TargetsT)
...
Disp=strcat('cycle=  ',0,' ,bestAll= ',strcat(num2str(CorPer)),' ,Fnum= ',strcat(num2str(size(Data,2))));
disp(Disp);
...
BestCycle=[BestCycle,CorPer];
Disp=strcat('cycle=  ',strcat(num2str(Cycle)),' ,bestFirst= ',strcat(num2str(CorPer)),' ,Fnum= ',strcat(num2str(length(flist))));
disp(Disp);
...
%     flist=flist(bestmemW==1); %%weighting
    ...
    Disp=strcat('cycle=  ',strcat(num2str(Cycle)),' ,best= ',strcat(num2str(bestval),' Fnum= ',num2str(length(flist))));
    disp(Disp);
    
%     %%%% cycle test evaluation
    flistB=flist(bestmem==1);
    DataF=Data(:,flistB(flistB~=0));
        [CorPer,cp]=ClassificationRate(DataF,Targets,KFindices,K,Kf,pTr,classMethod); %100-CorPer
%     %# performance
